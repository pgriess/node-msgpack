...
var unpack_buffer = mpBindings.unpack_buffer;

exports.pack = pack;
exports.unpack = unpack;
exports.unpack_buffer = unpack_buffer;
...
var BufferStream = function(s) {
    ...
    s.addListener('data', function(d) {
        ...
        // Consume messages from the stream, one by one
        while (self.buf && self.buf.length > 0) {
            var msg = unpack_buffer(self.buf);
            if (!msg) {
                break;
            }

            self.emit('msg', msg);
            if (unpack_buffer.bytes_remaining > 0) {
                self.buf = self.buf.slice(
                    self.buf.length - unpack_buffer.bytes_remaining,
                    self.buf.length
                );
            } else {
                self.buf = null;
...
sys.inherits(BufferStream, events.EventEmitter);
